...
def get_playlists(spotify_url: str) -> "tuple[str, dict[str, str]]":
    ...
    while data["next"]:
        r = requests.get(
            data["next"], headers={"Authorization": f"Bearer {spotify_key}"}
        )
        data = r.json()
    ...
    for track in tracks:
        ...
        try:
            ...
            if len(search_results):
                returned_tracks.update(
                    {
                        f"{song_name}": f"http://www.youtube.com/watch?v={search_results[0]}"
                    }
                )
